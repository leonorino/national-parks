...
enum class USState(val abbreviation: String, val fullName: String) {
    ...
    // Some parks are in territories
    DC("DC", "District of Columbia"), AS("AS", "American Samoa"), VI("VI", "Virgin Islands");

    companion object {
        fun fromAbbreviation(abb: String): USState? = entries.find { it.abbreviation == abb }
    }
}
